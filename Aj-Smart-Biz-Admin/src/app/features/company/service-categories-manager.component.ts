...
/**
 * The Service categories screen: how a business sorts the work it sells.
 *
 * A near-twin of the catalogue's own category editor, and deliberately a
 * separate screen rather than a shared one. They are two taxonomies with two
 * lifetimes: a salon has services and no catalogue, a furniture shop has a
 * catalogue and two services, and `/products/bridal` and `/services/bridal` are
 * different pages that would otherwise fight over one slug. What the two share
 * is the *tree*, and that lives in the API - the depth cap, the cycle check and
 * the promotion-on-delete are enforced there once.
 *
 * Three things follow from it being a tree rather than a list:
 *
 ...
 *                         unfiles its services rather than deleting either, and
 *                         the confirmation says so in those words
 *
 * The wording above the categories band on the website is edited here, saved as
 * part of the whole `services` settings blob - merged, so the section's own
 * heading and the booking hours survive a save from this screen.
 */
@Component({
  selector: 'app-service-categories-manager',
  ...
  templateUrl: './service-categories-manager.component.html',
  styles: [
    `
      :host { display: block; }
...
export class ServiceCategoriesManagerComponent {
  ...
  private readonly client = this.companies.serviceCategories();
  ...
  readonly tree = signal<ServiceCategory[]>([]);
  ...
  readonly editing = signal<ServiceCategory | null>(null);
  ...
  private load(): void {
    ...
    this.companies.functionalities().subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        this.loading.set(false);
        this.toast.error('Could not load your service categories', messageOf(error));
      },
    });
  }
  ...
  private loadTree(): void {
    this.companies.serviceCategoryTree(this.branchFilter() || undefined).subscribe({
   ...
   *
   * The API replaces a functionality's settings wholesale, and the `services`
   * blob is shared with the Services screen — which holds the section's wording,
   * the offers wording, the enquiry route, the page's menu name **and the
   * booking hours**. Sending only these three fields would clear all of it, so
   * what is already resolved is sent back alongside them.
   */
  saveCopy(): void {
    const current = (this.feature()?.settings ?? {}) as Partial<ServicesSettings>;
   ...
   */
  open(row: ServiceCategory | null, parent: ServiceCategory | null = null): void {
  ...
  save(): void {
    ...
    request.subscribe({
      next: () => {
        ...
        this.toast.success(row ? 'Service category updated' : 'Service category added');
        this.loadTree();
      },
      error: (error: HttpErrorResponse) => {
        this.saving.set(false);
        this.toast.error('Could not save the service category', messageOf(error));
      },
    });
  }

  toggle(row: ServiceCategory): void {
   ...
   * never touched.
   */
  move(row: ServiceCategory, direction: -1 | 1): void {
    ...
    this.client.reorder(siblings.map((item) => item.id)).subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        this.reordering.set(false);
        this.toast.error('Could not reorder the service categories', messageOf(error));
      },
    });
  }

  /** True when this row is the first or last of its own siblings. */
  isFirst(row: ServiceCategory): boolean {
    return this.siblingsOf(row)[0]?.id === row.id;
  }

  isLast(row: ServiceCategory): boolean {
    const siblings = this.siblingsOf(row);
    return siblings[siblings.length - 1]?.id === row.id;
  }

  private siblingsOf(row: ServiceCategory): ServiceCategory[] {
    const find = (nodes: ServiceCategory[]): ServiceCategory[] | null => {
   ...
   * obvious.
   */
  async remove(row: ServiceCategory): Promise<void> {
    const children = row.children?.length ?? 0;
    const products = row.serviceCount ?? 0;
    ...
    this.client.remove(row.id).subscribe({
      next: () => {
        this.toast.success('Service category deleted');
        this.loadTree();
      },
      error: (error: HttpErrorResponse) => this.toast.error('Could not delete the service category', messageOf(error)),
    });
  }
}

/** Depth-first, so a parent is always immediately above its children. */
function flatten(nodes: ServiceCategory[], depth = 1): FlatCategory[] {
...
function subtreeIds(nodes: ServiceCategory[], id: number): number[] {
  const collect = (node: ServiceCategory): number[] => [
    node.id,
    ...(node.children ?? []).flatMap(collect),
  ];

  const find = (list: ServiceCategory[]): ServiceCategory | null => {
