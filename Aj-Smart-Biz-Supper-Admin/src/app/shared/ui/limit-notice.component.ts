...
/**
 * The bar shown above a list once the company's plan has no room left, or the
 * plan behind it has lapsed. The wording comes straight from the API, so this
 * says exactly what a create request would have been refused with.
 */
@Component({
    ...
    @if (line(); as l) {
      @if (!l.canCreate) {
        <div class="limit" [class.hard]="l.reason !== 'limit_reached'">
          ...
          @if (showAction()) {
            <button type="button" class="btn btn-sm btn-primary" (click)="action.emit()">
              {{ actionLabel() }}
            </button>
          }
        </div>
...
export class LimitNoticeComponent {
  ...
  readonly actionLabel = input('Change plan');
