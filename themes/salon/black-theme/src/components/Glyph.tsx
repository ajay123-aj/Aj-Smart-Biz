 ...
 * Glyphs
 *
 * The platform's icon library, drawn rather than uploaded — so the set is
 * consistent across every tenant and every section, and costs no request: one
 * line weight, one 24-unit box, one cap and join.
 *
 * A tenant picks a name from the library in the admin; this is the half of that
 * bargain that decides what the name looks like. It is shared deliberately.
 * The Features / Benefits band and the Services section both let a company
 * choose a glyph, and two copies of this file would be two subtly different
 * drawings of `shield` on one page — which is exactly the thing a closed icon
 ...
 * falls through to `spark` rather than leaving a hole in a card.
 * ------------------------------------------------------------------ */
 ...
 */
export default function Glyph({ name }: { name: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
