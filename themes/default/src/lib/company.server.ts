 ...
 *
 * The endpoint lives in the backend — `GET /theme/company-details?domain=` in
 ...
 * whatever comes back.
 *
 * It is unauthenticated, like the `/theme/branding` endpoint the admin console
...
export async function getCompanyDetails(): Promise<ResolvedCompany> {
  ...
  try {
    const res = await fetch(`${API_URL}/theme/company-details?domain=${encodeURIComponent(domain)}`, {
