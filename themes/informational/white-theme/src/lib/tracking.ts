 ...
 * into a hundred leads. The API derives a stable-enough id of its own in that
 * case, and the tracker adopts whatever comes back.
 */
...
/** Records the id the server derived, so the next load is recognisably the same. */
export function adoptDeviceId(id: string | null | undefined): void {
  if (!id || readStore(window.localStorage, DEVICE_KEY)) return;
  writeStore(window.localStorage, DEVICE_KEY, id);
}
...
export async function sendVisit(options: TrackOptions = {}): Promise<void> {
  try {
    ...
    const body = (await response.json()) as { deviceId?: string | null };
    adoptDeviceId(body.deviceId);
  } catch {
