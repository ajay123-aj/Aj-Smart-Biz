/**
 * The shape `GET /theme/company-details` answers with, and the one helper both
 ...
 *
 * `platform` is the API's own key and reaches the template unmapped, so a
 * network added to the platform after this template was built still arrives —
 * `SocialLinks` falls back to a generic glyph rather than rendering nothing.
 *
 * `label` is always present: the API fills it with the platform's name where
 * the tenant did not override it, so the link always has an accessible name.
 */
...
export interface SiteFeatures {
  ...
  /**
   * The footer's social icons. Absent unless the plan grants `social_media`,
   * the tenant switched it on **and** added at least one link — so a footer
   * with no row is a tenant that is not publishing any, not a failure.
   */
  social: SocialFeature | null;
