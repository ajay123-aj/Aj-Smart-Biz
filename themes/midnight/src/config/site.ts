 ...
 * reviews and the articles all arrive from `/theme/company-details`. The copy
 * below is the **salon template's own**, and every line of it is a fallback for
 * a host that resolved to no tenant at all. Where a line should carry the
 * tenant's name it uses `{company}`, filled in at render time.
 *
 * Nothing here is a salon's actual content. A real salon's services and shelf
 * live in its own database rows, which is why this file has no price in it.
 */
...
export const CTA_BAND = {
  ...
  title: 'Tell {company} what you are after',
  body: 'A message is enough to start. Tell us roughly what you want done and when suits you, and we will come back with a stylist and a time.',
  button: 'Contact us →',
} as const;
...
export const SLIDES: Slide[] = [
  ...
  {
    eyebrow: 'The chair is yours',
    title: 'Cut, colour and care',
    body: 'Every appointment starts with a consultation, because the right cut depends on hair nobody has looked at yet. Nothing is decided before you sit down.',
  ...
  },
  {
    ...
    body: 'The person who consults is the person who cuts. You keep the same hands from the first appointment to the tenth, which is the only way a look stays yours.',
...
export const ABOUT_FALLBACK = {
  ...
  lead: 'A chair, a consultation, and somebody who has done this before.',
  paragraphs: [
    'We are a small studio that would rather take fewer appointments and finish each of them properly. Every booking is held long enough to consult first, so nothing is cut, coloured or treated before somebody has actually looked at your hair.',
    'Most of the people in our chairs were sent by somebody already in one. That is the only marketing figure we pay attention to.',
  ],
};
