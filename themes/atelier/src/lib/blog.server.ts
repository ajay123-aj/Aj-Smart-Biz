 ...
 * anything beyond `/theme/company-details`.
 *
...
export async function getBlogPage(params: {
  ...
  const { data, meta } = await get<BlogPostCard[]>(`/theme/blog${suffix}`);
...
export async function getBlogPost(slug: string): Promise<BlogArticle | null> {
  const { data } = await get<BlogArticle>(`/theme/blog/${encodeURIComponent(slug)}`);
  return data;
}
